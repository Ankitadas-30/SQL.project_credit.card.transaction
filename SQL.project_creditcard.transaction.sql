...
Data Preparation:
Changed the column names to lower case before importing data to sql server.
Changed the column names to suitable column names that can be easier to use.
Also replaced space within column names with underscore.
...
select gender, card_type,sum(amount) as total_amount 
from credit_card_transactions
...
select transaction_year,sum(cast(amount as BIGINT)) as total_amount 
from year_cte
...
from credit_card_transactions)
select top 5 cte_spends.*, round(total_spend*1.0/total_amount,4) * 100 as percent_contribution 
from cte_spends 
...
from cte)
select card_type,transaction_year, transaction_month,total_spend 
from cte_rn 
where rn=1
...
from credit_card_transactions)
select * from (select *, rank() over (partition by card_type order by running_total) as rn 
from cte
...
select *, lag(total_spend,1) over (partition by card_type,exp_type order by yt, mt) as previous_month_spend 
from cte)
select top 1 card_type, exp_type,yt,mt,total_spend-previous_month_spend as month_growth 
from cte_lag
